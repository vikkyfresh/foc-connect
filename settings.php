...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update = mysqli_query($conn, "UPDATE users SET phone = '$phone' WHERE id = $user_id");
    if($update) {
...
<head>
    ...
    <title>Settings - FoC Connect</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .settings-card { background: white; border-radius: 20px; padding: 28px; border: 1px solid #E8EDEC; }
        .form-group { margin-bottom: 24px; }
        label { display: block; font-weight: 600; color: #1A2E28; margin-bottom: 8px; }
        input, select { width: 100%; padding: 12px 16px; border: 1px solid #E8EDEC; border-radius: 12px; font-size: 15px; background: #F5F7F6; }
        input:focus { outline: none; border-color: #2E7D64; background: white; }
        .save-btn { background: #2E7D64; color: white; padding: 12px 24px; border: none; border-radius: 40px; font-size: 16px; cursor: pointer; }
        .message { background: #E8F5E9; color: #2E7D64; padding: 12px; border-radius: 12px; margin-bottom: 20px; }
            ...
            <div class="settings-card">
                ...
                <form method="POST">
                    <div class="form-group">
                        ...
                        <small style="color:#8A9B97;">Contact admin to change name</small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" name="phone" placeholder="Enter your phone number" value="<?php echo htmlspecialchars($user['phone'] ?? ''); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label>Department</label>
                        <input type="text" value="<?php echo htmlspecialchars($user['department_id'] ?? 'Not assigned'); ?>" disabled>
                    </div>
                    
                    <button type="submit" class="save-btn">Save Changes</button>
                </form>
                
                <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #E8EDEC;">
                    <h3 style="margin-bottom: 16px; font-size: 16px;">⚠️ Account Actions</h3>
